...
interface WebContainerContextValue {
  ...
  runCommand: (command: string, args: string[]) => Promise<{ process: any; exit: number }>;
  writeFile: (path: string, content: string) => Promise<void>;
  readFile: (path: string) => Promise<string>;
  removeFile: (path: string) => Promise<void>;
  createDirectory: (path: string) => Promise<void>;
  initializeProject: (projectId: string, userId: string, existingFiles?: any[]) => Promise<void>;
}
...
export function WebContainerProvider({ children }: { children: React.ReactNode }) {
  const [container, setContainer] = useState<WebContainer | null>(globalWebContainerInstance);
  ...
  const initializeProject = useCallback(async (projectId: string, userId: string, existingFiles?: any[]) => {
    ...
    const initPromise = (async () => {
      try {
        ...
        // Check if project already has files in Appwrite
        ...
        const existingFilesResponse = await databases.listDocuments(
          ...
          [Query.equal('projectId', projectId), Query.limit(10)]
        );

        const hasExistingFiles = existingFilesResponse.documents.length > 0;

        if (hasExistingFiles) {
          console.log('[WebContainer] 📂 Project has existing files, loading from Appwrite...');
          ...
          const { convertAppwriteFilesToFileSystemTree } = await import('@/lib/utils/fileSystemConverter');
          const fileSystemTree = convertAppwriteFilesToFileSystemTree(allFilesResponse.documents);
          
          // Mount existing files
          await mountFiles(fileSystemTree);
          console.log('[WebContainer] ✅ Loaded', allFilesResponse.documents.length, 'existing files');
        } else {
        ...
        devProcess.output.pipeTo(
          new WritableStream({
            write(data) {
              // Terminal will handle the output
              console.log('[DevServer]', data);
            },
          })
        );
        
        console.log('[WebContainer] ✅ Project initialized');
      } catch (err: any) {
