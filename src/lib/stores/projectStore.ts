...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  closeFile: (path) => {
    ...
    const newSelectedFile = selectedFile === path 
  ...
  deleteFile: (path) => {
    ...
    const newSelectedFile = selectedFile?.startsWith(path) ? null : selectedFile;
...
function renameFileInTree(files: FileNode[], oldPath: string, newPath: string): FileNode[] {
  return files.map(file => {
    ...
    if (file.children) {
      return { ...file, children: renameFileInTree(file.children, oldPath, newPath) };
    }
    return file;
  });
}
