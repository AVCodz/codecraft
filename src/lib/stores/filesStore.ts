...
export const useFilesStore = create<FilesState>((set, get) => ({
  ...
  loadFromLocalDB: (projectId: string) => {
    console.log('[FilesStore] 📂 Loading files from LocalDB for project:', projectId);
    const allFiles = localDB.getAll<ProjectFile>('codeCraft_files');
    ...
    console.log('[FilesStore] ✅ Loaded', projectFiles.length, 'files from LocalDB');

    // Build file tree if files exist
    if (projectFiles.length > 0) {
      const buildFileTree = async () => {
        const { buildFileTree } = await import('@/lib/utils/fileSystem');
        const tree = buildFileTree(projectFiles as any);
        const { fileTreeByProject } = get();
        set({
          fileTreeByProject: {
            ...fileTreeByProject,
        ...
        console.log('[FilesStore] 🌳 File tree built with', tree.length, 'root nodes');
      };
      buildFileTree();
    }
  },
  ...
  syncWithAppwrite: async (projectId: string) => {
    ...
    try {
      ...
      const files = response.documents as any as ProjectFile[];
      console.log('[FilesStore] 📥 Received', files.length, 'files from Appwrite');

      const tree = buildFileTree(files as any);
      console.log('[FilesStore] 🌳 Built file tree with', tree.length, 'root nodes');
      ...
      set({
        error: error.message || 'Failed to sync files',
        isSyncing: false
      });
    }
  },
