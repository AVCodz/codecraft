/**
 * ⚠️ SERVER-SIDE ONLY FUNCTIONS ⚠️
 *
 ...
 * - Server actions
 *
 * ❌ DO NOT use these functions in client-side code!
 *
 * For client-side operations, use Zustand stores which handle
 * LocalDB sync automatically:
 * - useFilesStore (for files)
 * - useMessagesStore (for messages)
 * - useProjectsStore (for projects)
 *
 * The stores follow the pattern:
 * User Action → Store → LocalDB + Appwrite
 * This ensures instant UI updates and proper sync!
 */
...
export async function createFile(data: CreateFileData): Promise<ProjectFile> {
  ...
  const resolvedLanguage = isFile
    ...
    : undefined;
  const resolvedSize = isFile
    ? data.size ?? Buffer.byteLength(resolvedContent ?? "", "utf-8")
    : 0;
...
export async function updateFile(
  ...
  const payload = {
    ...
    size:
      data.size ??
      (data.content ? Buffer.byteLength(data.content, "utf-8") : undefined),
    updatedAt: now,
  };
