...
export function Terminal({ className }: TerminalProps) {
  ...
  useEffect(() => {
    ...
    const terminal = new XTerm({
      ...
      theme: {
        ...
        selection: theme === 'dark' ? '#264f78' : '#0078d4',
    ...
    terminal.writeln('\x1b[1;32m│         Terminal Interface          │\x1b[0m');
    terminal.writeln('\x1b[1;32m╰─────────────────────────────────────╯\x1b[0m');
    terminal.writeln('');
    terminal.writeln('\x1b[1;36mℹ\x1b[0m This terminal shows build logs and command outputs.');
    terminal.writeln('\x1b[1;36mℹ\x1b[0m Files are automatically saved and processed by the AI.');
    terminal.writeln('');

    // Simulate some activity
    setTimeout(() => {
      terminal.writeln('\x1b[1;33m[INFO]\x1b[0m Project initialized successfully');
  ...
  return (
    <div className={`flex flex-col h-full bg-background ${className}`}>
      ...
      <div 
        ref={terminalRef} 
        className={`flex-1 ${isMaximized ? 'fixed inset-0 z-50 bg-background' : ''}`}
        style={{ 
